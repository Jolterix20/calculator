...
function num1Listener() {
	const grabNumberButtons = document.querySelectorAll(
		'.one, .two, .three, .four, .five, .six, .seven, .eight, .nine, .zero'
	)
	grabNumberButtons.forEach((button) =>
		button.addEventListener('click', num1Input)
	)
}
...
function num1Input(e) {
	...
	updateDisplay()
	if (calculator.num1.toString().length > 0) operationListener()
}
...
function removenum1Listener() {
	...
	grabNumberButtons.forEach((button) =>
		button.removeEventListener('click', num1Input)
	)
}
...
function num2Listener() {
	...
	grabNumberButtons.forEach((button) =>
		button.addEventListener('click', num2Input)
	)
}
...
function num2Input(e) {
	...
	updateDisplay()
	if (calculator.num2.toString().length > 0) equaltoListener()
}
...
function removenum2Listener() {
	...
	grabNumberButtons.forEach((button) =>
		button.removeEventListener('click', num2Input)
	)
}

function operationListener() {
	let grabOperationButtons = document.querySelectorAll(
		'.plus, .minus, .multiply, .divide'
	)
	grabOperationButtons.forEach((button) =>
...
function removeoperationListener() {
	...
	grabOperationButtons.forEach((button) =>
		button.removeEventListener('click', operation)
	)
}
...
function result() {
	...
	const resultafterOperation = operate(
		calculator.operation,
		Number(calculator.num1),
		Number(calculator.num2)
	)
...
function stripNum1() {
	if (
		calculator.num1.toString().length === 2 &&
		Number(calculator.num1) < 0
	) {
		removeoperationListener()
